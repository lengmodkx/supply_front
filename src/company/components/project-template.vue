  ...
  <div class="template-div">
    ...
    <Modal v-model="showTemplate" footer-hide title="自定义项目模板" width="360">
      <div class="add-template-box" v-if="showTemplate">
        ...
        <div style="margin-top:15px"><Input v-model="templateDes" maxlength="100" show-word-limit type="textarea" placeholder="请输入模板描述（必填）" /></div>
...
import { createTemplate, updateTemplate, deleteTemplate, initTemplate } from "../axios/api.js";
import { initTemplateRelation, createTemplateRelation, updateTemplateRelation, deleteTemplateRelation } from "../axios/api.js";
...
export default {
  ...
  methods: {
    initTemplate() {
      initTemplate({ orgId: localStorage.companyId }).then((res) => {
        if (res.result == 1) {
    ...
    handleBeforeUpload(file) {
      ...
      client.multipartUpload(this.fileName, file);
      return false;
    },
    random_string(len) {
      len = len || 32;
    ...
    deleteRelation(item) {
      let data = { relationId: item.relationId };
