import { enterTask, getmemberList, initEditTask, getTaskFiles, group, mView } from "../../axios/api.js";
const store = {
  namespaced: true,
  state: {
    allTasks: [],
  ...
  actions: {
    ...
    changeTask({ dispatch, commit }, data) {
      //任务数据改变时调用
      initEditTask(data).then((res) => {
        if (res.result === 1) {
          commit("editTask", res.data);
    ...
    initMemberList({ commit, state }, callback) {
    ...
    memberView({ commit }, data) {
      return new Promise((resolve) => {
        mView(data).then((res) => {
          if (res.result == 1) {
            commit("initTask", res.data);
            resolve();
          }
        });
      });
