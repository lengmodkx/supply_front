<template>
  <div class="member-box">
    <div class="member-wrap">
      <div class="members">
        ...
        <div class="member-left">
          <Input prefix="ios-search" class="search" @on-change="searchOrgProple" placeholder="搜索成员和部门" />
          <Tabs value="成员和部门" @on-click="clickTabs">
            <TabPane label="成员和部门" name="成员和部门">
              <div class="zzjg">
                <h3>成员</h3>
                <div class="filter-member" @click="changeMemberType(item)" :class="{ checked: memberType == item }"
                  v-for="(item, index) in allMemberType" :key="index">
                ...
                <div>
                  <div class="organization-title" v-if="departmentTreeNew.length>0">
                    ...
                    <span
                      class="organization-name">{{departmentTreeNew.length>0?departmentTreeNew[0].organizationName:''}}</span>
                  </div>
            ...
            <TabPane label="企业群组" name="企业群组">
              <ul class="group-ul">
                <li :class="{ checked: nowGroup.id === item.groupId }" class="group-list" v-for="(item, n) in groupData"
                  :key="n" @click="changeNowGroup(item)">
          ...
          <div class="addBtn" @click="groupStep1 = true;groupStep2 = false;direct = true" v-if="tabType === '企业群组'">
            创建群组
          </div>
        </div>
        <!--右侧显示成员-->
            ...
            <div v-else-if="nowType === '部门' && peopleList.length" class="branch-show">
              ...
              <Row class="branch-list" v-for="(i, n) in peopleList" :key="n">
                ...
                <div class="df">
                  <img :src="i.image" alt="" />
                  <!-- <div> -->
                  <div class="one-member-name">{{i.userName }}</div>
                ...
                <Poptip v-model="branchMenu">
                  ...
                  <div slot="content">
                    <div class="branch-title">
                      <span>
                        <Icon @click="branchMenuTitle = '群组菜单'" v-show="branchMenuTitle !== '群组菜单'"
                          type="ios-arrow-back" size="20" /></span>
                      {{ branchMenuTitle }}
    ...
    <Modal v-model="showAddPeople1" width="360" footer-hide>
      ...
      <addPeople @addPeople1="addPeople1" v-if="showAddPeople1" :invitUsers="allOrgPeople" :type="nowType"
    ...
    <Modal v-model="groupStep1" width="360" class-name="vertical-center-modal">
      ...
      <div class="craete-group-con">
        ...
        <div class="craete-group-depName">{{departmentTreeNew.length>0?departmentTreeNew[0].organizationName:'0'}}</div>
      </div>
      <div slot="footer">
      ...
      <div class="craete-group-con">
        ...
        <Row type="flex" justify="space-between" class="code-row-bg" v-if="direct">
          ...
          <Select v-model="changeDep" placeholder="部门" @on-change="selectDep" clearable>
            ...
            <Option v-for="item in departmentTreeNew" :value="item.partmentId" :key="item.partmentId">
    ...
    methods: {
      ...
      changePartment(partmentId, partmentName) {
        ...
        getBranchpeople(partmentId).then(res => {
          if (res.result == 1) {
            this.peopleList = res.data.organizationMembers;
            this.loading = false;
          }
        });
      },
