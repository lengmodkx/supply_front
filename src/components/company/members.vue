<template>
    <div class="member-box">
            ...
            <div class="members">
                        ...
                        <TabPane label="组织架构" name="组织架构">
                            <div class="zzjg">
                                ...
                                <div class="filter-member" @click="changeMemberType(item)"
                                     :class="{ checked: memberType == item }" v-for="(item, index) in allMemberType"
                                     :key="index">
                                    {{ item }}
                                </div>
                                ...
                                <div class="branch-head">
                                    ...
                                    <Poptip v-model="branch" transfer>
                                        ...
                                        <div slot="content">
                                            ...
                                            <Button long type="info" :loading="isCreateBranch"
                                                    :disabled="branchName == ''" @click="createBranch">创建
                                            </Button>
                                        </div>
                                    </Poptip>
                ...
                <div v-if="tabType === '组织架构'" class="member-right">
                    <header class="member-right-head">
                        <div style="font-size: 16px">{{ memberType }} · {{ peopleList.length ? peopleList.length : "0"
                            }}
                        </div>
                        <div class="header-right">
                            ...
                            <div v-if="nowType === '部门'" class="createZBM">
                                <Poptip v-model="sonBranch" transfer>
                                    ...
                                    <div slot="content">
                                        ...
                                        <p style="margin-bottom: 10px;color: gray;font-size: 14px">隶属于：{{ nowBranch.name
                                            }}</p>
                                        <Button long type="info" :loading="isCreateBranch"
                                                :disabled="sonBranchName == ''" @click="createBranch('子部门')">创建
                                        </Button>
                                    </div>
                                </Poptip>
                            </div>
                            ...
                            <div v-if="nowType === '部门'" class="more-bumen-opearte">
                                <Poptip v-model="branchMenu">
                                    ...
                                    <div slot="content">
                                        <div class="branch-title">
                                            <span><Icon @click="branchMenuTitle = '部门菜单'"
                                                        v-show="branchMenuTitle !== '部门菜单'" type="ios-arrow-back"
                                                        size="20"/></span>
                                            {{ branchMenuTitle }}
                    ...
                    <div class="scroll-box">
                        ...
                        <ul v-if="nowType === '成员' && peopleList.length">
                            <li class="one-member" v-for="(item, index) in peopleList" :key="index">
                                ...
                                <Poptip placement="bottom" transfer width="250"
                                        @on-popper-show="visibleChange(item.userEntity)"
                      ...
                      "
                                    ...
                                    <div slot="content" class="content" v-show="!visible">
                                        <ul class="org-role">
                                            <li v-for="(role, index) in roles" :key="role.roleId"
                      ...
                      "
                        ...
                        <ul v-else-if="nowType === '部门' && peopleList.length" class="branch-show">
                            ...
                            <li class="branch-list" v-for="(i, n) in peopleList" :key="n">
                                ...
                                <div class="operation">
                                    <Poptip>
                                        <div class="operation-title">{{ i.peopleList ? "管理员" : "成员" }}
                                            <Icon type="ios-arrow-down" size="20"/>
                                        </div>
                                        <div slot="content">
                                            <div class="operation-body-title">部门成员菜单
                                                <Icon type="md-close" @click="operationMenu = false"/>
                                            </div>
                                            <div class="operation-list">成员
                                                <Icon v-if="!i.peopleList" type="md-checkmark" size="18"/>
                                            </div>
                                            <div class="operation-list">管理员
                                                <Icon v-if="i.peopleList" type="md-checkmark" size="18"/>
                                            </div>
                                            <div class="operation-list">从部门移除成员</div>
                                            <div class="operation-list">
                                                <Poptip class="delete-alert" confirm title="确定从企业中移除成员？"
                                                        @on-ok="removePeopleOk" @on-cancel="removePeopleCancel">
                ...
                <div v-if="tabType === '企业群组'" class="member-right">
                    <header class="member-right-head">
                        <div style="font-size: 16px">{{ nowGroup.name }} · {{ groupPeople.length ? groupPeople.length :
                            "0" }}
                        </div>
                        <div class="header-right">
                            <!--添加成员 按钮-->
                            ...
                            <div class="more-bumen-opearte">
                                <Poptip v-model="branchMenu">
                                    ...
                                    <div slot="content">
                                        <div class="branch-title">
                                            <span><Icon @click="branchMenuTitle = '群组菜单'"
                                                        v-show="branchMenuTitle !== '群组菜单'" type="ios-arrow-back"
                                                        size="20"/></span>
                                            {{ branchMenuTitle }}
                                            <Icon @click="branchMenu = false" type="md-close" size="20"/>
                                        </div>
                                        <div class="branch-menu-con">
                                            ...
                                            <div class="bjbm" v-show="branchMenuTitle === '编辑群组'">
                                                ...
                                                <Button long type="info" :loading="isCreateBranch"
                                                        @click="changeGroupName">保存
                                                </Button>
                                            </div>
                                            <div class="scbm" v-show="branchMenuTitle === '删除群组'">
                                                <span>您确定要删除当前群组吗？</span>
                                                <Button long type="error" :loading="isCreateBranch"
                                                        @click="deleteGroupOk">确定
                                                </Button>
                                            </div>
                                        </div>
                                    </div>
        ...
        <Modal v-model="showAddPeople" width="360" footer-hide>
            ...
            <addPeople @add="addPeople" v-if="showAddPeople" :invitUsers="peopleList" :type="nowType"
                       :partmentId="partmentId"></addPeople>
        </Modal>
        <!--创建群组 框1 输入群组名称-->
