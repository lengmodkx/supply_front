<template>
  <div class="tag-con">
    <div class="tag-left">
      ...
      <ul class="tag-ul">
        <li class="tag-li" :class="{ checked: nowIndex === index }" v-for="(tag, index) in tags" :key="index">
    ...
    <div class="tag-right" v-if="nowTag">
      <div class="tag-right-title">
        ...
        <p>{{ nowTag.taskList.length }}项任务，</p>
        <p>{{ nowTag.scheduleList.length }}个日程，</p>
        <p>{{ nowTag.shareList.length }}篇分享，</p>
        <p>{{ nowTag.fileList.length }}个文件</p>
      </div>
        ...
        <div class="tag-right-view" v-if="nowTag.taskList.length">
          ...
          <ul class="right-view-ul">
            <li class="task-li" v-for="(i, n) in nowTag.taskList" :key="n">
              <div class="tiao"></div>
              <Checkbox v-model="i.taskStatus" @on-change="changeStatus(i.taskId, i.taskStatus)"></Checkbox>
              <!-- <img v-if="i.executorImage" src="" alt="" /> -->
              <Avatar :src="i.executorImg" v-if="i.executorImg" size="small" />
              <Icon v-else type="ios-contact" size="24" style="margin-right: 10px" />
              <p class="task-name">{{ i.taskName }}</p>
              <p class="gray-color">{{ i.groupName }}</p>
            </li>
          </ul>
        </div>
        <!--日程-->
    ...
    <Modal v-model="showModal" class="myModal" :mask-closable="false" footer-hide>
      <TaskDetail v-if="showModal" @close="showModal = false" :taskId="taskId"></TaskDetail>
    </Modal>
  </div>
</template>
<script>
import createTag from "./createTag";
import TaskDetail from "../project/pages/index/components/EditList.vue";
import { mapState, mapActions, mapMutations } from "vuex";
import { receclyTag, completeTask, cancelcompleteTask } from "@/axios/api";
...
export default {
  ...
  components: { createTag, TaskDetail },
  mounted() {
  ...
  methods: {
    ...
    changeStatus(taskId, status) {
      if (status) {
        completeTask(this.$route.params.id, taskId, 0);
      } else {
        //取消完成任务 请求
        cancelcompleteTask(this.$route.params.id, taskId, 0);
      }
    },
    // 关闭标签 菜单框
    closeOpearte(n) {
