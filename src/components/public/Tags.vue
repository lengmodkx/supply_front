<template>
  <div>
    <div class="list" ref="addIcon">
      <Tag type="border" closable v-if="taglist.length > 0" v-for="(item, index) in taglist" :key="index" :name="item.tagId" :color="item.bgColor" @on-close="handleClose">
        {{ item.tagName }}
      </Tag>
    ...
    <Modal v-model="showTag" :footer-hide="true" :width="300" @on-cancel="cancel">
      <p slot="header" style="text-align:center">
        <Icon type="ios-arrow-back" style="position:absolute;left:0;margin-left:10px;cursor:pointer" size="22" color="#999" @click="returnTag" v-if="showdiv1 == false" />
        <span>{{ tagTilte }}</span>
      </p>
      <div class="content">
        ...
        <div class="tagbox2 clearfix" v-if="showdiv2">
          ...
          <div class="createTag">
            ...
            <div class="btnBox">
              <Button class="crtBtn" type="primary" long :disabled="!tagName" @click="finish" :loading="loading">{{ isEdit == false ? "完成" : "创建" }}</Button>
            </div>
          </div>
        </div>
        <!-- 删除标签 页 -->
...
import { allTags, addnewTag, addTagAndBind, modifyTag, searchTags, delTag, removeInfoTag, bindingTag } from "../../axios/api";
export default {
