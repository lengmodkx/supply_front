  ...
  <div class="addCompanyWrap">
    ...
    <Tabs active-key="key1">
      <Tab-pane label="分享链接/申请码邀请" key="key1" v-if="tabOneShow" :index="one">
        ...
        <Button
          class="shareButton btn"
          type="primary"
          @click="copylink"
          data-clipboard-target=".tasklink"
        >复制邀请信息</Button>
      </Tab-pane>
      <Tab-pane label="手机/邮箱邀请" key="key2" :index="two">
        <div class="iphone">
...
import { verifyPhone, invitation } from "@/axios/companyApi";
import { linkInvitation } from "@/axios/api";
export default {
  props: ["tabOneShow"],
  name: "addPeople",
  data() {
  ...
  mounted() {
    ...
    linkInvitation(localStorage.companyId, "").then(res => {
      this.linkLoading=false
      if (res.result === 1) {
        this.shareWebsite = res.data.shortUrl;
      } else {
