<template>
   <Dropdown trigger="click" style="width:160px;">
        <span class="typeBtn" :class="{normal:(checkedName=='普通'),high:(checkedName=='紧急'),urgent:(checkedName=='非常紧急')}">{{checkedName}}</span>
        <DropdownMenu slot="list">
            <DropdownItem style="line-height:30px;"
             v-for="(item,index) in urgentList"
              :key="index"
              @click.native="choose(item.id,item.name)">
              <span class="typeBtn"
               :class="{normal:(item.name=='普通'),high:(item.name=='紧急'),urgent:(item.name=='非常紧急')}"
               >{{item.name}}</span>
