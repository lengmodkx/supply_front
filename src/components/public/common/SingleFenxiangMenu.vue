      ...
      <div slot="content" class="task-menuwrapper-content" v-if="initCom">
        ...
        <div class="task-menu-list" v-if="active === ''">
          <div
            ...
            "
          >
            <Icon type="md-color-filter" />编辑分享
          </div>
          <div class="menuItem" @click="topShare"><Icon type="ios-share-outline" />置顶分享</div>
          <div class="menuItem" @click="listItemClick('b', '复制到')"><Icon type="ios-paper-outline" />复制{{ name }}</div>
          <div class="menuItem" @click="listItemClick('a', '移动到')"><Icon type="ios-log-out" />移动{{ name }}</div>
          <div class="menuItem" @click="collectShare"><Icon type="md-clipboard" />{{ collect ? "取消收藏" : "收藏" + name }}</div>

          <div class="menuItem" @click="listItemClick('c', '移到回收站')"><Icon type="ios-trash-outline" />移到回收站</div>
          <div class="privacy">
  ...
  methods: {
    ...
    collectShare() {
      if (this.collect) {
        ...
        collectTask(this.projectId, this.data.id, "分享").then(res => {
          if (res.result === 1) {
            this.$Message.success(res.msg);
            this.collect = true;
