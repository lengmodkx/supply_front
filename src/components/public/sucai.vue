<template>
  ...
  <!-- <Modal v-model="tagHeader" :mask="false" fullscreen title="素材" footer-hide @on-cancel="closeTag"
     class="tab-content"> -->
  <div class="layout-right">
    <div style="background:#fff;padding:15px;min-height:100%" >
          ...
          <div class="main-content" :class="{'not-search':searched == ''}">
            <ul v-if="view === 'view' && allFile.length != 0" class="view-file-box">
              <li v-for="(file, index) in allFile" :key="index" @click="goNext(file.catalog, file.fileId, file)"
                :class="{ cur: file.catalog }">
                <Icon class="xiazai" v-if="!file.catalog" @click.stop="downLoad(file.fileId)"
                ...
                <div class="top-img" :data-id="file.fileId" v-else>
                  ...
                  <img v-if="file.fileThumbnail"
                    :src="`https://art1001-bim-5d.oss-cn-beijing.aliyuncs.com/${file.fileThumbnail}`" />
