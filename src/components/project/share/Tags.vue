<template>
  <Poptip placement="bottom" popper-class="addTag" v-model="Popvisible" @on-popper-show="popShow" @on-popper-hide="popHide" transfer>
    <div class="tagtitle" v-if="taglist.length==0">添加标签</div>
    <div class="list" v-if="taglist.length>0">
      ...
      <Icon type="ios-plus-outline" size="20" style="color:#2d8cf0;vertical-align:middle;" @click="Popvisible=true"></Icon>
    </div>
    <div slot="content">
        ...
        <div class="tag_input clearfix">
          ...
          <div class="add-icon fl">
            <Tooltip content="新建标签" placement="top">
              <Icon type="ios-plus-outline" size="20" @click="addTag"></Icon>
            </Tooltip>
          </div>
        </div>
        <div class="conBox">
              ...
              <li v-for="(color,i) in colorList" :key="i" @click="checkedColor=color">
                <div class="circle" :style="`background-color:${color}`">
                  <span class="markbox" v-if="color==checkedColor">
                    <Icon type="checkmark" class="mark"></Icon>
                  </span>
                </div>
              </li>
            </ul>
      ...
      <div class="div2 clearfix" v-if="showdiv2">
        ...
        <Input style="padding:0 14px;" v-model="tagName" placeholder="标签名称" />
        <div class="createTag">
          <ul class="tagcolor clearfix">
            <li v-for="(color,i) in colorList" :key="i" @click="checkedColor=color">
              <div class="circle" :style="`background-color:${color}`">
          ...
          <div class="btnBox">
            <Button type="ghost" long v-if="isEdit" @click="showdiv2=false;showdiv3=true">删除</Button>
            <Button class="crtBtn" type="primary" long :disabled="!tagName">{{isEdit?'完成':'创建'}}</Button>
          </div>
        </div>
      </div>
      <!-- 删除标签 页 -->
...
export default {
  ...
  computed: {
    ...
    computedTotalTag() {
      ...
      return this.totalTag.filter(v => this.$containStr(this.searchTag, v.name))
