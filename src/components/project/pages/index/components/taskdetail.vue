                  ...
                  <div class="object-field__right">
                    <Dropdown trigger="click" @on-click="taskStatusChange">
                      <div class="task-is-finish">
                        <svg-icon
                          name="task-not-finish"
                          class="task-not-finish"
                          v-if="task.taskStatus == false"
                        ></svg-icon>
                        <svg-icon
                          name="task-not-finish-hover"
                          class="task-not-finish-hover"
                          v-if="task.taskStatus == false"
                        ></svg-icon>
                        <svg-icon
                          name="task-finish"
                          class="task-finish"
                          v-if="task.taskStatus == true"
                        ></svg-icon>
                        <svg-icon
                          name="task-finish-hover"
                          class="task-finish-hover"
                          v-if="task.taskStatus == true"
                        ></svg-icon>
            ...
            <section class="detail-infos-note-view flex-horiz no-border-wrapper">
              ...
              <div class="note-section detail-infos-content flex-space">
                <div class="note-adder-set">
                  <div
                    class="add-note-handler"
                    @click="showEditor = true"
                    v-if="!showEditor"
                    v-html="task.remarks ? task.remarks : '待添加'"
                  ></div>
            ...
            <section class="detail-infos-priority-view no-border-wrapper">
              <div class="priority-container flex flex-vcenter">
                ...
                <div class="priority-wrap detail-infos-content">
                  <UrgentDropdown
                    v-on:priority="changePriority"
                    v-bind:checked-name="task.priority"
                  ></UrgentDropdown>
                </div>
              </div>
            </section>
                  ...
                  <div class="newSon clearfix">
                    <div class="sonInput fl">
                      <Input
                        ...
                        placeholder="输入子任务内容..."
                        autofocus
                        @keyup.enter.native="submitSontask"
                      />
                    </div>
                  </div>
                  <div class="btns">
              ...
              <div class="subtask-card-main">
                <div class="addfile" v-if="task.fileList">
                  <div class="file-lsit" v-for="(f, i) in task.fileList" :key="i">
                    <div class="file-img">
                      <img
                        v-if="images_suffix.indexOf(f.ext) > -1"
                    ...
                    <p class="file-time">{{ $moment(f.createTime).format("YYYY-MM-DD HH:mm") }}</p>
                  </div>
                  <div
                    style="width: 117px;height:117px;line-height: 117px;border: 1px solid #e5e5e5;display: flex;align-items: center;justify-content: center"
                    @click="showCommon = true"
                  >
    ...
    clone(id) {
      let url =
        "http://" + process.env.NODE_ENV == "development"
          ? "/tasks/" + id
          : process.env.VUE_APP_URL + "/tasks/" + id;
