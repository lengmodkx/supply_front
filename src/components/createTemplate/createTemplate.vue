<template>
  <div>
    <div class="modal-header">
      <div class="top-nav df jsb ac">
        <div class="template-properties df">
          ...
          <a class="edit-template" @click="modal = true">
            <Icon type="md-create" />
            <span>编辑</span>
          </a>
        </div>
        <div class="df ac">
          ...
          <Icon type="md-close" size="24" style="margin-right:15px" @click="close" />
        </div>
      </div>
    </div>
    <div class="task">
    ...
    <Modal v-model="shwoCreate" :width="440" :mask-closable="false" footer-hide>
      ...
      <creat-list v-if="shwoCreate" :taskMenuId="taskMenuId" :taskGroupId="taskGroupId" @close="shwoCreate = false"> </creat-list>
    </Modal>
...
import { getRelation } from "../../axios/template.js";
export default {
  components: {
    draggable,
    TaskMenu,
  ...
  mounted() {
    getRelation({templateId:this.templateId}).then(res=>{
