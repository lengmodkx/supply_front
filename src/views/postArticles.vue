            ...
            <div class="title">
                <Dropdown trigger="click" style="margin-left: 5px" @on-click="changType">
                    ...
                    <DropdownMenu slot="list">
                        <DropdownItem v-for="item in classList" :key="item.acId" :name="item.acName">{{item.acName}}
                        </DropdownItem>
                    </DropdownMenu>
                </Dropdown>
                <div class="line"></div>
            ...
            <div class="video-content" v-if="clickTypeAcName=='视频'">
                <div class="video-upload">
                    <Upload type="drag" :action="host" :format="['mp4','avi']" accept="video/mp4,video/avi"
                        :show-upload-list="true" :before-upload="videoBeforeUpload" ref="videoUpload" :max-size="5120"
                    ...
                    <div class="video-list df ac" v-for="(item,index) in videoList">
                        <template>
                            ...
                            <div class="video-list-cover">
                                <Icon type="ios-trash-outline" @click.native="handleRemoveVideo(item,index)" size="20">
                                </Icon>
                            </div>
                        </template>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                } else if (this.param.coverShow == 1 && this.param.coverImages == undefined) {
                    this.$Message.warning('请选择封面');
                } else {
                    article(this.param).then(response => {
            ...
            saveVideo() {
                ...
                if (this.videoParam.videoName == '') {
                    ...
                    article(this.videoParam).then(response => {
                        if (response.result == 1) {
                            this.$Message.success('发布成功');
                            this.$router.push("/org/" + localStorage.companyId);
                        }
            ...
            saveHeadlines() {
                ...
                if (this.headlinesParam.headlineContent == '') {
                    ...
                    article(this.headlinesParam).then(response => {
                        if (response.result == 1) {
                            this.$Message.success('发布成功');
                            this.$router.push("/org/" + localStorage.companyId);
                        }
