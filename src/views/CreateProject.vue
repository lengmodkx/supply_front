<template>
  <div class="contentPro">
    ...
    <div class="create-project-time">
      <DatePicker placeholder="开始时间" :value="startTime" type="datetime" @on-change="startDate" :options="options1">
      </DatePicker>
      <DatePicker placeholder="结束时间" :value="endTime" type="datetime" @on-change="endDate" :options="options2">
      </DatePicker>
    </div>

    <Button class="submitBtn" type="primary" size="large" :loading="loading"
      :disabled="proName == '' || startTime == '' || endTime == ''" @click="create">
...
<script>
  import { createProject, getTemplate } from "@/axios/api";
  ...
  export default {
    ...
    mounted() {
      getTemplate(localStorage.companyId).then(res => {
        this.proTemplate = res.data
      });
    },
    methods: {
      create() {
